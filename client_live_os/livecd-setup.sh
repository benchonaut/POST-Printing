...
sed 's~printserver.local:3142/~~g' -i /etc/apt/sources.list /etc/apt/sources.list.d/*
sed 's~http://~http://printserver.local:3142/~g;s~https://~http://printserver.local:3142/~g' -i /etc/apt/sources.list /etc/apt/sources.list.d/* 
...
time apt-get -y install $(apt list --upgradable|grep -v "Listing"|grep -v -e linux-image)
