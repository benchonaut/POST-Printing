#!/bin/sh
find /tmp/ -mindepth 1 -maxdepth 1 -newermt '-20 seconds' -type f -name ".status.json"|grep .status.json -q  && { cat /tmp/.status.json ; date > /tmp/.printstatus_pre_exit; } ;
find /tmp/ -mindepth 1 -maxdepth 1 -newermt '-20 seconds' -type f -name ".status.json"|grep .status.json -q  && exit 0
test -e /tmp/.printer_status_update_running && sleep 3
find /tmp/ -mindepth 1 -maxdepth 1 -not -newermt "-30 seconds" -type f -name "/tmp/.printer_status_update_running" -delete & 
...
touch /tmp/.printer_status_update_running &
...
for CARD in $(echo {01..16});do
    ...
    i=$CARD;
    find /tmp/.printerstatus/ -mindepth 1 -maxdepth 1 -newermt '20 seconds ago' -type f -name "status_card.$i"|grep -q "status_card.$i"|| (
...
for LABEL in $(echo {01..16});do
    ...
    find /tmp/.printerstatus/ -mindepth 1 -maxdepth 1 -newermt '20 seconds ago' -type f -name "status_label.$i"|grep -q "status_label.$i"|| (
        (echo -n '"label-'$i'":"';
        [[ -z "IP" ]] && echo "NO IP FOUND"
...
test -e /tmp/.printer_status_update_running && rm /tmp/.printer_status_update_running & 
