...
if(isset($_POST) AND !empty($_POST)) 
	...
	if(isset($_POST['NoRotate']))
		...
		if ($_POST['NoRotate'] == 'Front')	 
			{ $execute='';for ($a=1;$a<16;$a++ ) { $num=sprintf("%02d",$a);$execute=$execute.'lpadmin -p CARD'.$num.' -o FPageRotate180=OFF;' ; } ; exec($execute); } 
...
print('<form method="POST" action="'.curPageURL().'?action=NoRotFront" onchange="document.getElementById(\'NoRotFront\').form.submit();setTimeout(location.reload(),3000)"> <button name="NoRotate" id="NoRotFront" value="Front">Straight Front</button></form>');
print('</td><td><form method="POST" action="'.curPageURL().'?action=RotFront" onchange="document.getElementById(\'RotFront\').form.submit();setTimeout(location.reload(),3000)"> <button name="Rotate" id="RotFront" value="Front">Rotate Front</button></form>');
print('</td><td><form method="POST" action="'.curPageURL().'?action=NoRotBack" onchange="document.getElementById(\'NoRotBack\').form.submit();setTimeout(location.reload(),3000)"> <button name="NoRotate" id="NoRotBack" value="Back">Straight Back</button></form>');
print('</td><td><form method="POST" action="'.curPageURL().'?action=RotBack" onchange="document.getElementById(\'RotBack\').form.submit();setTimeout(location.reload(),3000)"> <button name="Rotate" id="RotBack" value="Back">Rotate Back</button></form>');
print('</td></tr></table>');
